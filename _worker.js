...
const serviceName = "killer"; // Ganti dengan nama workers kalian
...
const KV_PROXY_URL = "https://raw.githubusercontent.com/FoolVPN-ID/killer/refs/heads/main/kvProxyList.json";
const PROXY_BANK_URL = "https://raw.githubusercontent.com/FoolVPN-ID/killer/refs/heads/main/proxyList.txt";
const DNS_SERVER_ADDRESS = "8.8.8.8";
const DNS_SERVER_PORT = 53;
const PROXY_HEALTH_CHECK_API = "https://id1.foolvpn.me/api/v1/check";
const CONVERTER_URL = "https://killer-tool.azurewebsites.net/api/v1/convert";
    ...
    try {
      ...
      if (upgradeHeader === "websocket") {
        ...
        if (url.pathname.length == 3 || url.pathname.match(",")) {
          ...
          let kvProxy = await env.killer.get("kvProxy");
          if (kvProxy) {
            kvProxy = JSON.parse(kvProxy);
          } else {
            kvProxy = await getKVProxyList();
            env.killer.put("kvProxy", JSON.stringify(kvProxy), {
              expirationTtl: 3600,
            });
          }
    ...
    <script>
      ...
      function checkRegion() {
        for (let i = 0; ; i++) {
          ...
          const res = fetch(
            "https://killer-tool.azurewebsites.net/api/v1/regioncheck?config=" + encodeURIComponent(configSample)
          ).then(async (res) => {
            if (res.status == 200) {
